...
const getCards = async (params, updateState) => {
  ...
  const notes = await axios.get(`/notes/${collection}/${note}`);
  for (let data of JSON.parse(notes.data)) {
    ...
    if (data[3].length !== 0) {
      ...
      for (let i of data[3]) {
        pre = document.createElement('pre');
        // comment
        if (i.comment) pre.innerHTML = `<span class="comment">${i.comment}</span>`;
        // input
        else if (i.input) pre.innerHTML = `<span class="input">${i.input}</span>`;
        // output
        else if (i.output) pre.innerHTML = `<span class="output">${i.output}</span>`;
        // images
